<template>
  <div class="nav-side" :class="{customizing: isSort}" ref="navSide">
    ...
    <div class="nav-list" ref="list">
      ...
      <div v-if="isDrag && replaceItem === data.length" :class="['sortable-item']">
...
<script>
import scrollMixin from "./smooth-scroll";
...
export default {
  ...
  watch: {
    ...
    contentRows() {
      console.log('contentRows 变化')
      if (this.contentRows && this.contentRows.length > 0) {
        console.log('contentRows 存在:', this.contentRows[0])
        this.init()
      }
    }
  },
  computed: {
    ...mapState({
      sortKeys: state => state.contentRowStore.sortKeys,
  ...
  methods: {
    ...
    getOffsetTop(element) {
      let top, clientTop, scrollTop, doc = document.documentElement, body = document.body
...
.nav-side
  ...
  .gotop
    ...
    .btn_gotop
      ...
      border-radius 4px
  .tip
    background url(../../assets/images/drag.png) 0 0 no-repeat
    position absolute
    left -117px
    top 0
    width 117px
